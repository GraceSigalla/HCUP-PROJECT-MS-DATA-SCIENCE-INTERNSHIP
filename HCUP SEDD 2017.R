...
# Age groups and percentage

age_var <- ny_sedd1 %>% mutate( 
...
died_var <- ny_sedd1 %>% group_by(DIED) %>% summarise(
  percent = 100 * n()/nrow(ny_sedd1)) %>% mutate(DIED = case_when(
      DIED == 0 ~ "Lived",
      DIED == 1 ~ "Died"
))
...
gender_var <- ny_sedd1 %>% group_by(FEMALE) %>% summarise(
  percent = 100 * n()/nrow(ny_sedd1)) %>% mutate(FEMALE = case_when(
    FEMALE == 0 ~ "Male",
    FEMALE == 1 ~ "Female"
))
...
# Gender is denoted as FEMALE, 0 being male and 1 female

# Pay Percentages

pay_var <- ny_sedd1 %>% group_by(PAY1) %>% summarise(
  percent = 100 * n()/nrow(ny_sedd1)) %>% mutate(PAY1 = case_when(
    PAY1 == 1  ~ "Medicare",
    PAY1 == 2  ~ "Medicaid",
...
diagnosis_info <- select(ny_sedd1, I10_DX_Visit_Reason1, I10_DX_Visit_Reason2, I10_DX1,
...
# Saving NY SEDD Data frames as CSV (those containing null values)

# NY_SEDD
write.csv(ny_sedd,"//files.drexel.edu/encrypted/SOPH/UHC/SchnakeMahl_HCUP/HCUP Data_NY//2017 NY SEDD.csv", row.names = TRUE)

# NY_SEDD1
write.csv(ny_sedd1,"//files.drexel.edu/encrypted/SOPH/UHC/SchnakeMahl_HCUP/HCUP Data_NY//2017 NY SEDD Sample.csv", row.names = TRUE)

# Diagnosis Info
write.csv(diagnosis_info,"//files.drexel.edu/encrypted/SOPH/UHC/SchnakeMahl_HCUP/HCUP Data_NY//2017 Diagnosis Information.csv", row.names = TRUE)

# Mutated Diagnosis Info
write.csv(diagnosis_vars,"//files.drexel.edu/encrypted/SOPH/UHC/SchnakeMahl_HCUP/HCUP Data_NY//2017 Mutated Diagnosis Information.csv", row.names = TRUE)

# Grouped Diagnosis Info
write.csv(diagnosis_vars_group,"//files.drexel.edu/encrypted/SOPH/UHC/SchnakeMahl_HCUP/HCUP Data_NY//2017 Grouped Diagnosis Information.csv", row.names = TRUE)

# Percentage Diagnosis Info
write.csv(diagnosis_vars_perc,"//files.drexel.edu/encrypted/SOPH/UHC/SchnakeMahl_HCUP/HCUP Data_NY//2017 Percentage Diagnosis Information.csv", row.names = TRUE)

# ILI Diagnosis Info
write.csv(diagnosis_vars_fil,"//files.drexel.edu/encrypted/SOPH/UHC/SchnakeMahl_HCUP/HCUP Data_NY//2017 ILI Diagnosis Information.csv", row.names = TRUE)

# Combined race ethnicity
write.csv(race_eth,"//files.drexel.edu/encrypted/SOPH/UHC/SchnakeMahl_HCUP/HCUP Data_NY//2017 Race and Ethnicity.csv", row.names = TRUE)
...
library(imputeTS)
diagnosis_info1 <- na_mean(diagnosis_info)
view(diagnosis_info1)

#Then round up 
diag_info <- diagnosis_info1 %>% 
...
flu <- c("J1000", "J1001", "J1008","J101","J102","J1081",'J1082', 'J1083','J1089',
         'J1100','J1108', 'J111','J112','J1181','J1182','J1183','J1189','J09X1','J09X2',
         'J09X3','J09X9')
  
  
flu_like <- c('J069', 'J399', 'J200', 'J201','J202', 'J203', 'J204', 'J205',
              'J206', 'J207', 'J208', 'J209', 'J210', 'J211', 'J218', 'J219', 'J40','B012',
              'B052', 'B0681', 'B250', 'J120', 'J121', 'J122', 'J123', 'J1281', 'J1289', 'J129',
              'J440', 'J441', 'J470', 'J471')
  

diagnosis_cat <- diag_info %>% mutate(dates = as.factor(as.Date(with(diag_info, paste(AYEAR,AMONTH,"01",sep="-")),"%Y-%m-%d")),
                                           diagnosis = ifelse(I10_DX_Visit_Reason1 %in% flu | I10_DX_Visit_Reason2 %in% flu, "ILI",
                                                          ifelse(I10_DX_Visit_Reason1 %in% flu_like | I10_DX_Visit_Reason2 %in% flu_like, "ANY ILI","Other")))
...
diag_cat1 <- diagnosis_cat %>% group_by(dates, diagnosis) %>%
  summarize(total= n()) %>% ungroup()
...
diag_cat2 <- diag_cat1 %>% group_by(dates)  %>% mutate(
...
# Filter out "other" diagnosis
diag_cat_ili <- diag_cat2 %>% filter(diagnosis != "Other")
view(diag_cat_ili)


#Chart

ggplot(data=diag_cat_ili, aes(dates,percentage, colour = diagnosis)) + 
...
# Recreating table with ILI disparities

# Any ILI diagnosis Total

total_ili <- ny_sedd %>% mutate(date = as.factor(as.Date(with(ny_sedd, paste(AYEAR,AMONTH,"01",sep="-")),"%Y-%m-%d")),
                                      diagnosis_variable = ifelse(I10_DX_Visit_Reason1 %in% flu_diag | I10_DX_Visit_Reason2 %in% flu_diag, "ILI",
                                                             ifelse(I10_DX_Visit_Reason1 %in% flu_like_diag | I10_DX_Visit_Reason2 %in% flu_like_diag, "ANY ILI","Other")))
...
total_ili1 <- total_ili %>% group_by(diagnosis_var) %>%
  summarize(total= n()) %>% ungroup()
...
ny_sedd4 <- ny_sedd3 %>% group_by(date)  %>% mutate(
  percent = total / sum(total)* 100) %>% arrange(desc(date))


view(ny_sedd4)

total_individual_diagnosis <- ny_sedd4 %>% group_by(diagnpsis_var) mutate(
  sumation = 
)

#ny_sedd1 <- ny_sedd[nysedd_samp,]
