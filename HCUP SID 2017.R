...
# LINKING SID FILE WITH AHA FILE

# Read the AHA DTA file

AHA_sid17 <- read.dta13("NY_SID_2017_AHAL.dta")
View(AHA_sid17)

# Linking the two files

sid_aha17 <- left_join(ny_sid_mod, AHA_sid17, by = c("DSHOSPID" = "DSHOSPID"))
...
county_tab <- select(P0_NAME, STATE, I10_DX_Admitting)
...
# Creating diagnosis variables

county_vars <- county_tab %>% mutate(diagnosis_var = ifelse(I10_DX_Admitting %in% flu_diag, "ILI",
ifelse(I10_DX_Admitting %in% flu_like_diag, "ANY ILI","OTHER")))
...
county_group <- county_vars %>% group_by(PO_NAME, diagnosis_var) %>%
  summarize(total= n()) %>% ungroup()
...
county_perc <- county_group %>% group_by(PO_NAME,STATE)  %>% mutate(
